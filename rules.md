...
Spaces separate units, and each unit is evaluated first. Literals
separated by spaces stay together as one vector, so `1 2 3` is one
value. Within a unit, functions take everything to their right as the
right argument, and arithmetic has no precedence. Parentheses also
group. Monads take one argument, dyads two.
...
A unit that ends in a function is a left section. It is that unit’s
expression with its right argument missing.

``` apl
f←32+1.8× ⋄ f 100    ⍝ 212
```

Operators bind before function application. Names hold values or
operators, resolved at execution time. Arrays contain numbers,
characters, functions and arrays.

`→` separates pipeline stages, evaluated left-to-right. Each stage uses
ordinary APL binding. Each function receives the previous result as its
right argument. `←` takes the whole pipeline as its value.
...
Predicates, positions, tally and shape return exact integers. Iota and
random integer generation preserve exact arguments. Mixed arrays retain
each element’s domain.
...
[Brackets](glyphs/brackets.qmd) write lists: `[a b c]`. A list of
literals can leave them out: `1 2 3`. Nothing else forms a list.
Brackets round one item only group it, so `[Y]` is `Y`, and `[Y;]` is a
one-item list. Only `⊂Y` encloses, and `↑` retrieves the first item.
Enclosure always adds an array layer.
...
as 1. Equal dimensions agree; unit dimensions expand, including to zero.
Unit axes remain. Other mismatches: `LENGTH`.
...
`⍤` applies it to trailing cells. `/ \ ⌽ ,` default to the last axis,
and `⌿ ⍀ ⊖ ⍪` to the first. Structural functions also accept axis lists.
...
Tolerance is not transitive; search/grouping uses the first matching
representative.
