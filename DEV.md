...
- `selection.rs`: temporary labels for selective assignment. The binder marks their data flow and permits only selection functions; masks still read real user bindings. Nested labels retain their storage and whole-item paths. Prototype labels preserve empty-cell structure.
...
`Array` stores private shape/data/prototype fields with immutable `Rc` sharing. Storage is `Integer(Vec<i64>)`, `Float(Vec<f64>)` or `Mixed(Vec<Element>)`. Checked constructors select compact storage without promoting adjacent elements. `as_integers()` and `as_floats()` expose borrowed slices. `at()` and `elements()` yield owned elements without materializing the whole array. A cached numeric-domain summary combines every element's domain, including nested summaries and empty prototypes. Shape and tally inspect this summary in constant time. Scalar shape is `[]` with one element; zero dimensions preserve other dimensions. Nonempty construction derives its prototype; empty construction requires a prototype. Prototype filling memoizes shared nested subarrays and compacts storage without invoking a fresh prototype derivation. Array elements cannot contain functions.
...
`eval_with` installs per-evaluation `EvalOptions`. Clone its `InterruptHandle` to another thread or supply a timeout. Checks run at binding/call boundaries and inside long interpreted/primitive loops. Tight bounded float kernels keep their existing slice paths; native-library calls and individual BigInt operations are not preempted. `Worker.eval` allows a grace period before killing an unresponsive process. Killing loses the session; requests are never replayed. Native Python sessions expose deadlines, not cross-thread access or an asynchronous interrupt method.
...
Numeric semantics are Rust-owned. Python copies exact integers through PyO3 integer conversions, constructs `Fraction` values in the Python adapter, and copies non-real values through `PyComplex::from_doubles`. No Python numeric objects enter the core. JSON emits exact integers as arbitrary-sized JSON integers using serde_json's `arbitrary_precision` feature. Fractions retain tagged decimal-string components, complex values a tagged numeric pair. Reference-interpreter cases compare equal numeric values across exact/float domains because Dyalog has no corresponding explicit exact domain. Rust core tests assert the exact representation, prototypes and compact buffers separately.
