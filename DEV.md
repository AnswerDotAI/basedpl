...
- `number.rs`: concrete Float/Exact values; checked input construction, explicit monadic/dyadic arithmetic, mixed promotion, comparison, display, and checked structural conversion. Representation is private; every Number is finite/canonical, so array construction never revalidates/reduces it. Exact integers use `BigRational` with denominator 1. Add complex support with real consumers, not placeholder variants.
...
The numeric policy and current limits are documented in README.md. Serde is a frontend dependency; PyO3 remains optional and is constrained to the tested 0.29 compatibility line. JSON requests use a typed `code: String` object; an object check prevents Serde's positional-array struct decoding from broadening the protocol. Keep JSON and Python conversions separate: they implement different external contracts.

No collector is selected. Lexical frames are a stack with non-owning parent indices, separate from dynamic call/handler state. Nested functions see live lexical bindings, not snapshots. The current subset cannot export functions or put them in arrays, dfn assignment is local, and dfn results are arrays. Thus a reference to a local frame cannot outlive its active invocation; frames pop on success and error. Recheck this proof before adding namespaces, nonlocal assignment, function exports/results, or tail-frame reuse. It is not evidence that the entire planned endpoint needs no escaping closures.

Catch-all `0::` guards keep a shallow binding-map checkpoint from before the condition executes. Restoring it removes newly introduced names as well as restoring previous values. This is rollback state, not snapshot closure semantics. A journal would add machinery without a demonstrated need at this scale. Guard handlers are popped before execution and unwind dynamically through ordinary calls; output is retained. Specific error numbers, interrupts, general guards, and tail-call handling remain later work. Unsupported-feature errors are not caught.
...
Numeric semantics are Rust-owned. Python copies exact integers directly through PyO3's big-integer conversion and constructs `Fraction` values in the Python adapter; no decimal-string conversion or Python rational objects enter the core. JSON independently encodes numerator/denominator strings. Complex support and exact arithmetic are required for the high-school/first-year-college endpoint, not optional release-scope decisions.
