...
import json, math, queue, subprocess, threading
...
class Worker:
    ...
    def _read(self):
        try:
            for line in self.process.stdout: self._replies.put(json.loads(line))
    ...
    def request(self, payload, timeout=None, grace=1):
        ...
        with self._request_lock:
            ...
            try:
                ...
                try: reply = self._replies.get(timeout=None if timeout is None else timeout+grace)
                except KeyboardInterrupt:
                    self.interrupt()
                    try: self._replies.get(timeout=grace)
