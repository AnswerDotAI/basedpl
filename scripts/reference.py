"""Import the checked-out reference examples without executing upstream programs.

The JSONL files are editable acceptance cases, not generated at test time. Import into
a new directory when updating upstream: never overwrite reviewed case statuses.
"""
import base64
import csv
import json
...
def april_file(path, source):
    ...
    for start, end in sorted(forms):
        ...
        if not tokens[first][0].startswith('"'):
            ...
            continue
        if head == 'provision':
            cases.append(record(source, line, lisp_string(tokens[first][0]), upstream=raw, role='setup'))
            continue
        if head == 'is-error':
