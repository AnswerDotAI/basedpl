...
Windows receive prototype fill at edges.
...
the start, negative at the end. Atop with `⊢`, as in `+/⍤⊢`, discards
`⍺`. A two-row specification gives sizes then movements.
...
- `RANK`: specification not a scalar, vector or two-row matrix, or more
  axes than `Y`
