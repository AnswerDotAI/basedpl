...
`C𝒫X`: evaluate **constant-first** coefficients at `X`. [Decode](decode.md) uses the opposite order.

```apl
1x 2x 3x𝒫0x 1x 2x   ⍝ 1x 6x 17x
```

`𝒫C`: leading coefficient and numerical roots, as a nested pair. Ignores trailing zeros. Constants have no roots.
...
`(M R)𝒫X`: `M××/X-R`. `⊂R` assumes `M=1`; use `⊂,R` for one root. Monadic `𝒫` gives coefficients.
...
```apl
(⊂2 3⍴1x 2x 0x 1x 0x 2x)𝒫⊂3x 4x  ⍝ 25x
