...
fn inverse_power() {
    ...
    fails(Domain, &["({⍵}⍣¯1)3", "(0∘×⍣¯1)0", "(⍸⍣¯1)3 1", "(⍸⍣¯1)0", "(2∘⊥⍣¯1)¯1", "(2 2∘⊥⍣¯1)5"]);
}

#[test]
fn inverse_combinations() {
    equiv! {
        "((+∘-)∘2)⍣¯1⊢3" => "5",
        "2 ((+∘-)⍨⍣¯1)3" => "5",
        "((÷⍤-)∘2)⍣¯1⊢0.5" => "4",
        ...
        "10 20 (+\\[1]⍣¯1)2 2⍴11 22 14 26" => "2 2⍴1 2 3 4",
        "(+\\⍣¯1)⍬" => "⍬",
        "10 (+\\⍣¯1)13" => "3",
    ...
    fails(Domain, &["(×\\⍣¯1)1 0 0", "0 (×\\⍣¯1)0 0", "(-⍛+⍣¯1)0", "((⊢+⊢)⍣¯1)4"]);
    fails(Length, &["1 2 3 (+\\⍣¯1)2 3⍴⍳6"]);
}
...
fn grading() {
    equiv! {
        ...
        "9007199254740992x 9007199254740993x⍸9007199254740992 9007199254740994" => "1x 2x",
...
fn character_arithmetic() {
    ...
    fails(Domain, &["1-'a'", "'a'+'b'", "'a'×2", "'a'+1.000000000000001", "'a'+∞", "'a'+1j1", "'a'+1r2", "(⎕UCS 55295)+1", "(⎕UCS 1114111)+1", "(⎕UCS 0)-1"]);
}
...
fn matrix_axes_scan_and_cell_assembly() {
    equiv! {
        ...
        "2 +\\5" => "7",
        "10 20 +\\2 3⍴⍳6" => "2 3⍴11 13 16 24 29 35",
        "10 20 30 +⍀2 3⍴⍳6" => "2 3⍴11 22 33 15 27 39",
        "10 20 {⍺+⍵}\\[2]2 3⍴⍳6" => "2 3⍴11 13 16 24 29 35",
        "{⍺,⍵}\\1 2 3" => "1 (1 2) (1 2 3)",
    ...
    assert_eq!(r.output, ["1 2", "¯1 3", "1 ¯1 ¯4"]);
    let r = Session::new().eval("r←10 {⎕←⍺ ⍵ ⋄ ⍺-⍵}\\1 2 3");
    assert_eq!(r.output, ["10 1", "9 2", "7 3"]);
    fails(Length, &["(,10)+\\1 2 3", "10 20 30+\\2 3⍴⍳6", "10 20 30+\\2 0⍴0"]);
...
fn each_commute_and_reduction() {
    ...
    fails(Domain, &["÷¨⍬", "1÷¨⍬", "2¨3"]);
    fails(Domain, &["f←2¨", "f←2∘3", "f←+⍥3", "f←+⌺×"]);
    fails(Index, &["¯1⊃¨0⍴⊂1 2 3", "¯1{⍺⊃⍵}¨0⍴⊂1 2 3"]);
