...
from basedpl import apl, fn, plus
...
def test_ipython_inspection_and_completion():
    ...
    apl('twice←'+source)
    f = fn('twice')
    shell.user_ns.update(apl=apl, f=f, fn=fn)
    ...
    for line in ['apl["tw', 'apl.fn("tw', 'apl("tw', 'fn("tw']:
    ...
    del shell.user_ns['apl'], shell.user_ns['f'], shell.user_ns['fn']
