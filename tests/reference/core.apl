...
fs←+‿× ⋄ f←3⊃(0↑fs)‿(0↑fs)‿÷ ⋄ 6 f 3
2

⍝ — Arrays and functions can share a mixed vector
fs←+‿× ⋄ result←(⊂1 2 3),fs[2] ⋄ back←2⊃result ⋄ 2 back 3
6
...
⍝ — A pick path cannot continue through a function
1 1⊃+‿×
⍝ error: DOMAIN ERROR
...
choose←{1+⍺>⍵}◶(-‿÷) ⋄ 12 choose 3
4

⍝ — An agenda branch may itself return a function
choose←1◶({⊃⍵}‿⊢) ⋄ f←choose (+‿×) ⋄ 2 f 3
5
...
choose←{⎕←9 ⋄ 2}◶({⎕←1 ⋄ 1÷0}‿{⎕←2 ⋄ ⍺-⍵}) ⋄ 10 choose 3
7
⍝ ⎕: 9\n2
...
(2 1⍴1 2)<1 3⍴1 2 3
2 3⍴0x 1x 1x 0x 0x 1x

⍝ —
(2 1⍴1x 2x)<1 3⍴1x 2x 3x
2 3⍴0x 1x 1x 0x 0x 1x
...
⍝ —
a←(1 2)(3 4) ⋄ (⊃a)←7 8 9 ⋄ 1⊃a
7 8 9

⍝ — Replacing a disclosed scalar with a vector introduces nesting
a←1 ⋄ (⊃a)←3 4 ⋄ a
⊂3 4

⍝ — Disclose-each selects the first element of each nested vector
a←(1 2)(3 4) ⋄ (⊃¨a)←(5 6)(7 8) ⋄ a
((5 6)2)((7 8)4)
...
↑[1 3](2 3⍴⍳6)(2 3⍴6+⍳6)
2 2 3⍴1 2 3 7 8 9 4 5 6 10 11 12
...
¯2⍕3.25 ¯3.25 325 0.0325
' 3.3E0 ¯3.3E0 3.3E2 3.3E¯2'

⍝ — Matrix formatting aligns exponential fields by column
¯2⍕2 2⍴3.125 0.002 1000 20
↑' 3.1E0 2.0E¯3' ' 1.0E3 2.0E1 '
...
⍕1E¯6 1E¯7 1E16 1E17
'0.000001 1E¯7 10000000000000000 1E17'
...
1x 2x 3xⓅ2 2⍴0x 1x 2x 3x
2 2⍴1x 6x 17x 34x

⍝ — Evaluate the factored form 2(x-1)(x-3)
(2x (1x 3x))Ⓟ0x 1x 2x 3x
6x 0x ¯2x 0x
...
f←(⊂2 3⍴1x 2x 0x 1x 0x 2x)∘Ⓟ ⋄ f∂⊂3x 4x
⊂6x 8x
...
ℙ⍳8
2x 3x 5x 7x 11x 13x 17x 19x

⍝ — Prime lookup preserves shape and accepts repeated, unordered indices
ℙ2 2⍴10000 1 10000 2
2 2⍴104729x 2x 104729x 3x
...
1ℙ341550071728321x 3825123056546413051x
0x 0x

⍝ — Factor a semiprime whose two factors are both large
Ⓠ1000000016000000063x
1000000007x 1000000009x
...
⍝ — An inverse trailing-axis scan needs one seed per row
1 2 3 (+\⍣¯1)2 3⍴⍳6
⍝ error: LENGTH ERROR
...
3x 5x 7x⌹3 2⍴1x 1x 1x 2x 1x 3x
1x 2x
...
G←2 3⍴('ABC' 1)('DEF' 2)('GHI' 3)('JKL' 4)('MNO' 5)('PQR' 6) ⋄ H←('' '*' @((1 2)1)((2 3)2))G ⋄ (((1 2)1)⊃H)(((2 3)2)⊃H)
'' '*'
...
((2 1)1 2)⊃2 3⍴('ABC' 1)('DEF' 2)('GHI' 3)('JKL' 4)('MNO' 5)('PQR' 6)
'K'

⍝ — Empty coordinates can repeatedly pick a scalar without changing it
(5⍴⊂⍬)⊃10
10
...
u←•ucs ⋄ u 2 2⍴'A⍳λ😀'
2 2⍴65x 9075x 955x 128512x

⍝ —
•UCS 2 2⍴65x 9075x 955x 128512x
2 2⍴'A⍳λ😀'
...
⍝ —
'UTF-8'•UCS 'Æ😀'
195x 134x 240x 159x 152x 128x

⍝ —
('UTF-8' 0)•UCS 195 134 240 159 152 128
'Æ😀'
...
⍝ — Bracket indexing retains the selected character vector's enclosure
'abc' 'def' 'ghi'[2]
⊂'def'
...
⍝ — Reduction sums nested vectors and retains their enclosure
+/(1 2)(3 4)
⊂4 6
...
10 20 +\2 3⍴⍳6
2 3⍴11 13 16 24 29 35

⍝ — Leading-axis scan takes one seed per column
10 20 30 +⍀2 3⍴⍳6
2 3⍴11 22 33 15 27 39

⍝ — Axis-qualified dfn scan follows the same seed agreement
10 20 {⍺+⍵}\[2]2 3⍴⍳6
2 3⍴11 13 16 24 29 35
...
⍝ — Mix pads a scalar cell rather than extending its value
↑1 (2 3)
2 2⍴1 0 2 3
...
⍝ — Empty nested reduction uses a conforming enclosed identity
+/0⍴(1 2)(3 4)
⊂0 0
...
⍝ — A vector scan needs a scalar seed, not a singleton vector
(,10)+\1 2 3
⍝ error: LENGTH ERROR
...
⍝ — Seed agreement is checked even when the scan axis is empty
10 20 30+\2 0⍴0
⍝ error: LENGTH ERROR
...
x←1 ⋄ y←1+8E¯15 ⋄ z←1+16E¯15 ⋄ x y⍳z
2x
...
1x 2x~1000000000000001r1000000000000000
1x 2x
...
⍝ — Not-match distinguishes a scalar from a singleton vector
1≢,1
1x
...
⍝ — An n-wise reduction one longer than the axis yields an empty result
3+/1 2
⍬

⍝ — Leading-axis n-wise reduction combines adjacent rows
2+⌿2 3⍴⍳6
1 3⍴5 7 9
...
⍝ — Prototype-mode Pick still checks coordinate rank
⍬⊃¨⊂2 3⍴⍳6
⍝ error: RANK ERROR

⍝ — An n-wise window more than one longer than the axis is invalid
4+/1 2
⍝ error: LENGTH ERROR
...
outer←⌝ ⋄ times←× ⋄ 1 2 times outer 3 4
2 2⍴3 4 6 8
...
⍝ — Equality has no monadic valence, even with an empty argument
=⍳0
⍝ error: SYNTAX ERROR
