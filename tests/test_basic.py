...
from basedpl import apl, AplError, Array
...
def test_native_calls_and_explicit_output():
    ...
    assert apl('1 ⋄ ⎕←2 ⋄ 3').py == 3
    assert apl('1 ⋄ ⎕←2 ⋄ 3', 'explicit').output == ['2']
    assert apl('+/x', x=a).py == Fraction(7, 3)
...
def test_clear_restores_starting_workspace():
    ...
    apl(x=1)
    apl(']clear')
    with pytest.raises(AplError, match='VALUE'): apl('x')
    assert apl('1 2', 'repl').output == ['1 2']
    apl(']box on')
    apl(']box reset')
    assert apl('1 2', 'repl').output == ['1 2']
    with pytest.raises(AplError, match='SYNTAX'): apl(']clear x')
...
def test_bindings_functions_and_output(capsys):
    ...
    assert apl(']Display 1 2', 'explicit').output and capsys.readouterr().out == ''
...
def test_numpy_inputs_and_copies():
    ...
    assert apl('x', x=np.arange(3, dtype=np.int8)).np.dtype == np.int64 and apl('x', x=np.ones(2, np.float32)).np.dtype == np.float64
...
def test_interrupts_and_threads(capsys):
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for call, interrupt, error in [(apl, apl.interrupt, AplError), (apl, ctrl_c, KeyboardInterrupt), (lambda src: apl(src, 'explicit'), ctrl_c, KeyboardInterrupt)]:
...
def test_without_numpy():
    code = "from basedpl import apl; import sys; assert apl('1+2').py == 3.; assert 'numpy' not in sys.modules"
