...
Adjacent values form a strand, preserving every value. [Bracketed](glyphs/brackets.qmd) values never strand. `⊂Y` and `[Y]` enclose, and `↑` retrieves the first item. Enclosure always adds an array layer.
...
Indices and axes start at 1. Negative positions count from the end. Counts and indices must be exactly integral, without tolerance. [Index](glyphs/squad.qmd) `⌷` selects positions, and `∞` selects a whole axis. After an array, [dot indexing](glyphs/dot.qmd) writes the same selection: `m.(∞ 2)` is `∞ 2⌷m`.

[Rank](glyphs/rank.qmd) `⍤` with an enclosed operand selects axes, as in `+⍤[2]` and `+/⍤[1]`. `/ \ ⌽ ,` default to the last axis, and `⌿ ⍀ ⊖ ⍪` to the first. Structural functions also accept axis lists or fractional insertion positions: `0.5` before axis 1, `1.5` between axes 1 and 2.
