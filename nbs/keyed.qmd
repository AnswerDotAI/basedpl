...
A parenthesised literal whose items are all written as `key:value` builds one keyed vector. Keyed vectors display in this form. If any item is not written as `key:value`, the literal is an ordinary vector of its items.
...
Plain assignment through a dot path adds missing keys. Missing names along the path become records.
...
Plain named assignment appends missing rows or columns. New matrix cells receive the array's prototype before assignment.
...
M←'city' 'month':[0]2 3⍴⍳6
N←'month':[0]10 20 30
M+N                           ⍝ 'city' 'month':[0][11 22 33 ⋄ 14 25 36]
⍳[0]N+M                      ⍝ 'month' 'city'
M+⍉M                         ⍝ 'city' 'month':[0][2 4 6 ⋄ 8 10 12]
```
...
```apl
M←'city' 'month':[0]2 3⍴⍳6
M+10 20                       ⍝ 'city' 'month':[0][11 12 13 ⋄ 24 25 26]
A←('city' ⋄ 2):[0]2 3⍴1
B←('product' ⋄ 2):[0]4 3⍴2
...
```apl
M←'city' 'month':[0]('LA' 'NY' ⋄ 'Jan' 'Feb' 'Mar'):2 3⍴⍳6
