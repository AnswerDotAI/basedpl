...
Hold Alt and press a key from the [keyboard layout](keyboard.qmd): **Alt-h/j/k/l** gives `← ↓ ↑ →`, **Alt--** gives `×`, **Alt-=** gives `÷`. Alt-Shift-a/w gives the operands `⍶ ⍹`. Chords insert literal characters, including inside strings and comments.
...
Before `⊸` is Alt-u, or `` `before`` then Tab. After `⟜` is Alt-t, or `` `after`` then Tab. Use `` `pi`` for `π` and `` `sqrt`` for `√`.
...
Each glyph has one name, the one the [glyph reference](glyphs.qmd) uses. The REPL also matches search words such as `reshape` for `⍴`. It always shows the name: `` `resh `` lists `⍴ rho r`.
...
Inside brackets and parentheses, a line break counts as a space. Rows of an array need `⋄`.
...
| `]box ?` | Show settings |
| `]box reset` | Restore the starting display settings |
